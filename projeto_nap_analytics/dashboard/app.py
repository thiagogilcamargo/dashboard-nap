...
# ============================================================
# KPIs (VISÃO GERAL) - CORRIGIDO
# ============================================================
st.subheader("📊 Visão Geral")

# Calcular as médias antes
...
with col3:
    st.metric("🎯 Necessidade de apoio", f"{necessidade:.1f}/10")

with col4:
    st.metric("🏫 Suporte percebido", f"{suporte:.1f}/10")

with col5:
    st.metric("📊 Gap", f"{gap:.1f}")

with col6:
    st.metric("🎯 Intenção de uso", f"{intencao:.1f}/10")

with col7:
    st.metric("👀 Conhecem não usaram", f"{pct_conhece:.0f}%")
...
if 'score_intencao' in df.columns and df['score_intencao'].mean() > 0:
