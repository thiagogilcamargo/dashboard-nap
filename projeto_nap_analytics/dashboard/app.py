...
# Tratar NaN
necessidade = 0 if pd.isna(necessidade) else necessidade
suporte = 0 if pd.isna(suporte) else suporte
gap = 0 if pd.isna(gap) else gap
intencao = 0 if pd.isna(intencao) else intencao

# Criar 4 linhas de 4 colunas cada (mais organizado)
...
if 'score_suporte' in df.columns and df['score_suporte'].notna().any():
    scores_data.append({"Dimensão": "Suporte", "Score": df['score_suporte'].mean()})
if 'score_gap' in df.columns and df['score_gap'].notna().any():
    scores_data.append({"Dimensão": "Gap", "Score": df['score_gap'].mean()})
...
if scores_data:
    ...
    st.info("📊 Nenhum score disponível para exibição.")
...
if 'score_gap' in df.columns and df['score_gap'].notna().any():
    ...
    if gap_medio > 3:
        st.warning(f"⚠️ **Alerta:** O gap entre necessidade ({necessidade:.1f}) e suporte percebido ({suporte:.1f}) é de {gap_medio:.1f} pontos.")
    elif gap_medio > 1:
